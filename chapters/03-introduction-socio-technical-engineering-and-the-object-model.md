...
**One-line:** Students learn why objects - not procedures - are the right mental model for business software, by running and breaking a working application before they write one.

## Module Overview

By the end of this module, you will add one concrete capability to the semester project: students learn to read running code as a business model before writing new code. The library example is the main path through the text. Inventory and healthcare scheduling follow the same design shape in the exercises.

## Prerequisites

Module 0 setup and basic programming concepts from any language.

## Learning Objectives

- (Understand) Explain why a business problem maps more naturally to objects than to a list of instructions.
- (Apply) Modify a provided Java program and predict the effect before running it.
- (Analyze) Distinguish a compilation error from a runtime error.
- (Evaluate) Assess which parts of a simple Java modification task are safe to delegate to AI.

## Opening Case

The library checkout program runs. It has a `Book`, a `Patron`, and a checkout action. Then the student changes one line and the program either refuses to compile or runs with the wrong patron attached to the wrong book.
...
Object-oriented programming starts with a claim about the world: business systems are made of things with identity, state, and behavior. Java classes are the way this book makes that claim executable.
...
AI may explain code and error messages. It may not write new code for submission.
...
**Situation.** Trace the library checkout from source file to running output. A `Book` knows title and availability. A `Patron` knows name and borrowed books. A checkout connects them. The first bug appears when the connection is made in the wrong place.
...
Modify the checkout program for two books, predict output, run it, classify any errors, and write an AI Use Disclosure comparing your reading to an AI explanation.

The lab must include running evidence, a short explanation of the design choice, and the AI Use Disclosure described below.

## How To Use AI

**What AI does well here:** AI may explain code and error messages. It may not write new code for submission.

**Concrete prompt that works:**

```text
I am working on Module 1: Introduction: Socio-Technical Engineering and the Object Model in INFO 5100.
My domain is [library / inventory / scheduling].
My current specification is: [paste your written specification].
Help only within this boundary: AI may explain code and error messages. It may not write new code for submission.
Do not produce work that violates the phase gate: If the prompt asks AI to produce Java you will submit, stop. If it asks AI to explain Java you already have, proceed.
```
...
**Phase gate:** If the prompt asks AI to produce Java you will submit, stop. If it asks AI to explain Java you already have, proceed.

**Module CLAUDE.md constraint:**

```text
You are assisting a graduate Java student in Module 1: Introduction: Socio-Technical Engineering and the Object Model.
...
You can now students learn to read running code as a business model before writing new code. You also have one more AI boundary: not a ban, but a rule for keeping verification in the student's hands.

## Key Terms

- **class:** define this term in the context of the semester project before using it in lab work.
- **object:** define this term in the context of the semester project before using it in lab work.
- **state:** define this term in the context of the semester project before using it in lab work.
- **behavior:** define this term in the context of the semester project before using it in lab work.
- **compilation error:** define this term in the context of the semester project before using it in lab work.
- **runtime error:** define this term in the context of the semester project before using it in lab work.
- **stdout:** define this term in the context of the semester project before using it in lab work.

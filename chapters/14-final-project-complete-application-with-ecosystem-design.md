...
<!-- → [TABLE: five-component handoff checklist — columns: component, what it is, what "done" means for this component, the failure mode if it is missing; rows: running code (no crash on examiner's inputs), source code (navigable by someone else), tests (evidence of critical behaviors), AI Use Disclosures (specific task/prompt/verification/judgment), defense (three decisions in strong format) — student uses this as a pre-submission gate] -->
...
<!-- → [FIGURE: layered architecture diagram of the complete application — five horizontal bands stacked bottom to top: (1) Supply Side: Book, Author, Catalog — "models the world, no UI knowledge"; (2) Transaction Layer: CheckoutTransaction, PatronSession — "uses entities, does not create them"; (3) Persistence: saveToFile / loadFromFile — "survives restart, independent of UI"; (4) View Layer: ListView, Button, Label — "displays model state, owns no state"; (5) Event Layer: handlers, lambdas, controller calls — "translates, does not execute"; arrows between layers pointing upward labeled "depends on" — student should be able to map every class in their project to one of the five bands before the defense] -->
...
<!-- → [INFOGRAPHIC: three-question audit form — three labeled boxes with example answers filled in: (1) "What did you ask AI to do, and what did it produce?" → example: "Asked AI for saveToFile method; it produced a version that crashed on missing file"; (2) "What did you verify, and how?" → example: "Ran with empty catalog directory; confirmed silent return; confirmed CSV format after save"; (3) "What did you decide that AI could not?" → example: "Decided missing file = first run = silent, not error; AI did not know deployment context" — a gate symbol below all three labeled "submit only after all three are specific" — caption: vague disclosures are not disclosures] -->
...
<!-- → [TABLE: design decision defense template — columns: question, weak answer (what it reveals), strong answer (what it demonstrates); rows: "What does this component do?" (weak: restates the code / strong: names the requirement it satisfies), "What alternatives existed?" (weak: "I didn't think of any" / strong: names at least one with its cost), "Why this choice for your requirements?" (weak: "it's standard practice" / strong: names the specific requirement that made this better), "What would you change?" (weak: "nothing" / strong: names a trade-off that a different requirement would flip) — caption: the strong answers are the defense; the weak answers are the absence of one] -->
...
<!-- → [TABLE: three-level AI delegation audit examples — columns: answer quality, what it names, what it reveals to the examiner; rows: level 1 "validation logic because AI didn't know my requirements" (names reason, generic), level 2 "checkout state machine because AI allowed double-checkout" (names specific AI failure and violated requirement, better), level 3 "authentication because AI stored passwords in plain text" (names security requirement + failure + human decision, best) — caption: the quality of the answer reveals whether the student understood both the code and the requirement, or only one] -->
...
<!-- → [TABLE: three test category checklist — columns: category, what it tests, example test description, what it proves to the examiner; rows: persistence tests (round-trip survival, "save three books, reload, assert field equality", that the data layer works across restarts), state transition tests (business rule enforcement, "attempt double-checkout, assert rejection", that the model enforces domain rules), edge case tests (error handling under abnormal input, "load malformed CSV, assert no crash", that the error handling from earlier modules actually fires) — student uses this to confirm all three categories are covered before the demo] -->
...
<!-- → [INFOGRAPHIC: two-panel "impression vs evidence" contrast — left panel labeled "An impression": terminal output screenshot showing the application running, captioned "ran it a few times, seemed fine"; right panel labeled "Evidence": a test method asserting persistence round-trip plus a state transition test asserting double-checkout rejection, captioned "named the requirement, named the observation" — caption: the examiner asks "how do you know?"; evidence answers the question; an impression does not] -->
...
<!-- → [INFOGRAPHIC: course capability arc — a horizontal timeline from "Module 1" to "Module 14" with five labeled capability milestones above the line: (1) Class and object model; (2) Supply-side and transaction layers; (3) Persistence and exception handling; (4) GUI and event handling; (5) Defense and AI accountability — and a single caption below the timeline: "The running program is evidence the code works. The explanation is evidence you built it." — student should be able to locate every project component on this arc before the final defense] -->

...
**One-line:** Students learn to model the entities that provide resources in a business system - the objects that exist before any user interacts with them.

## Module Overview

By the end of this module, you will add one concrete capability to the semester project: the chapter separates durable business resources from user transactions. The library example is the main path through the text. Inventory and healthcare scheduling follow the same design shape in the exercises.

## Prerequisites

Module 4 debugging workflow and class/object fluency.

## Learning Objectives

- (Apply) Design a supply-side object model with at least three classes.
- (Analyze) Distinguish entity, relationship, and transaction objects.
- (Apply) Implement model relationships as a runnable Java application.
- (Evaluate) Assess an AI-generated class design against specified requirements.

## Opening Case

A library catalog exists before any patron arrives. If the checkout transaction creates the catalog, the design is backwards. The resource side of the system needs its own model.
...
Supply-side modeling identifies the objects that make the business possible before a user action occurs: books, products, providers, rooms, inventory, service slots. These objects have relationships and constraints independent of the current screen.
...
<!-- → [FIGURE: entity relationship transaction classification table for Module 5, showing the core concept, the Java artifact that carries it, the AI assistance boundary, and the human verification step.] -->

## Worked Example

**Situation.** A `Catalog` contains `Book` objects and exposes search. It does not know which patron is logged in. Checkout uses the catalog, but the catalog is not a checkout transaction.
...
<!-- → [TABLE: Module 5 verification table with columns: concept, Java artifact, what AI may do, what the student must verify, evidence before submission; rows should include the library example plus inventory and scheduling variants.] -->
...
Add a supply-side model to the semester project with at least three classes, five preloaded objects, and one search or lookup behavior.

The lab must include running evidence, a short explanation of the design choice, and the AI Use Disclosure described below.

## How To Use AI

**What AI does well here:** AI may generate class stubs from a written specification. It may not invent the specification.

**Concrete prompt that works:**

```text
I am working on Module 5: Modeling the Supply Side in INFO 5100.
My domain is [library / inventory / scheduling].
My current specification is: [paste your written specification].
Help only within this boundary: AI may generate class stubs from a written specification. It may not invent the specification.
Do not produce work that violates the phase gate: For each class, write what it knows, what it does, and how it relates to other classes before asking AI for stubs.
```
...
**Phase gate:** For each class, write what it knows, what it does, and how it relates to other classes before asking AI for stubs.

**Module CLAUDE.md constraint:**

```text
You are assisting a graduate Java student in Module 5: Modeling the Supply Side.
...
You can now the chapter separates durable business resources from user transactions. You also have one more AI boundary: not a ban, but a rule for keeping verification in the student's hands.

## Key Terms

- **entity:** define this term in the context of the semester project before using it in lab work.
- **relationship:** define this term in the context of the semester project before using it in lab work.
- **transaction:** define this term in the context of the semester project before using it in lab work.
- **catalog:** define this term in the context of the semester project before using it in lab work.
- **array:** define this term in the context of the semester project before using it in lab work.
- **loop:** define this term in the context of the semester project before using it in lab work.
- **abstraction:** define this term in the context of the semester project before using it in lab work.

## Bridge Question

The system has resources. Who is allowed to use them?

...
<!-- → [INFOGRAPHIC: three-panel failure gallery — each panel shows one of the three breakdown scenarios (startup preload, duplicate search logic, simultaneous transactions) with a brief label describing what breaks and why the transaction-creates-resource design cannot handle it — student should read this as a checklist of the design's actual failure modes, not an abstract warning] -->
...
<!-- → [FIGURE: annotated Catalog class diagram showing the class boundary, the private books[] array and count field inside it, addBook and findByIsbn and searchByTitle methods on its surface, and an arrow from an external Patron object calling findByIsbn — student should see that the Patron reaches into the Catalog but the Catalog has no arrow pointing back] -->
...
<!-- → [FIGURE: object graph showing Author objects as separate heap nodes, Book objects holding Author[] references pointing to those nodes, and a contrast panel showing the "names-in-a-book" anti-pattern where each Book contains duplicate string data with no shared identity — caption: the relationship on the left is modeled; the duplication on the right is implied] -->
...
<!-- → [TABLE: three-domain comparison table — columns: domain, entity classes, relationship objects, transaction classes; rows: library (Book/Author/Catalog, AuthorBook, CheckoutTransaction), inventory (Product/Supplier/Warehouse, SupplierRelationship, PurchaseOrder), healthcare scheduling (Provider/Room/ServiceType, ProviderQualification, Booking) — student should see the structural identity across domains and recognize the pattern they are learning is not library-specific] -->
...
<!-- → [TABLE: four-row artifact checklist — columns: artifact name, what it is, the specific code pattern that carries it, the failure mode if it is missing; rows: entity class, collection class, preload block, query method — student should use this as a self-check before submitting the lab] -->
...
<!-- → [INFOGRAPHIC: annotated main() block with supply-side lines highlighted in one color and demand-side lines in another, with a horizontal dividing line between them labeled "separation boundary" — a callout points to the catalog.searchByTitle call above the boundary with the label "separation test: this runs without any Patron" — student should be able to apply the same color-coding to their own main before submitting] -->
...
<!-- → [INFOGRAPHIC: horizontal three-stage flow — (1) Student writes specification: "Book knows title, isbn, available; relates to Author[]" [human only, no AI]; (2) Student gives spec to AI: "Generate a stub for this class" [AI as code generator]; (3) Student evaluates stub against spec: does the output match what I specified? [human verification, gate before proceeding] — a red X on any arrow that skips stage 1, labeled "this is where plausible Java substitutes for verified Java"] -->
...
<!-- → [TABLE: two-column comparison — "Transaction-creates-resource design" vs "Supply-side model" — rows: can a librarian search without a patron? / can an admin add books before opening? / can two patrons transact simultaneously without interference? — each cell shows Yes or No with one sentence explaining why — student should see that these are not aesthetic preferences but capability differences] -->

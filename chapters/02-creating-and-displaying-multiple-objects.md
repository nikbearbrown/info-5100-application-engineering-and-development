...
**One-line:** Students learn to instantiate multiple objects from one class and trace how Java manages them in memory.

## Module Overview

By the end of this module, you will add one concrete capability to the semester project: the chapter builds the reference-versus-object mental model needed for later debugging. The library example is the main path through the text. Inventory and healthcare scheduling follow the same design shape in the exercises.

## Prerequisites

Module 1 class/object vocabulary and ability to run a Java program.

## Learning Objectives

- (Understand) Explain the distinction between a class and an object.
- (Apply) Write a Java class with attributes and methods and instantiate two objects.
- (Analyze) Trace variable values across two objects.
- (Apply) Extend the semester project's first class with two additional attributes.

## Opening Case

Three printouts show the same person's name and three different ages. The class is the same. The objects are not. The question is where those values live.
...
A class is a blueprint. An object is one constructed instance of that blueprint. A reference variable is not the object itself; it is the handle through which code reaches the object.
...
AI may quiz you on code you wrote or identify possible design issues. It may not write the first class body.
...
<!-- → [FIGURE: reference-to-object memory trace for Module 2, showing the core concept, the Java artifact that carries it, the AI assistance boundary, and the human verification step.] -->

## Worked Example

**Situation.** Create `patronA` and `patronB` from the same `Patron` class. Change `patronA.name`. Then inspect why `patronB.name` is unchanged. The trace separates local variables, references, and heap objects at a conceptual level.
...
<!-- → [TABLE: Module 2 verification table with columns: concept, Java artifact, what AI may do, what the student must verify, evidence before submission; rows should include the library example plus inventory and scheduling variants.] -->
...
Write a `Book` class with four attributes, instantiate three distinct books, display them, and add the first project class for the chosen domain.

The lab must include running evidence, a short explanation of the design choice, and the AI Use Disclosure described below.

## How To Use AI

**What AI does well here:** AI may quiz you on code you wrote or identify possible design issues. It may not write the first class body.

**Concrete prompt that works:**

```text
I am working on Module 2: Creating and Displaying Multiple Objects in INFO 5100.
My domain is [library / inventory / scheduling].
My current specification is: [paste your written specification].
Help only within this boundary: AI may quiz you on code you wrote or identify possible design issues. It may not write the first class body.
Do not produce work that violates the phase gate: Write the class yourself first, then ask AI to ask you five questions about it.
```
...
**Phase gate:** Write the class yourself first, then ask AI to ask you five questions about it.

**Module CLAUDE.md constraint:**

```text
You are assisting a graduate Java student in Module 2: Creating and Displaying Multiple Objects.
...
You can now the chapter builds the reference-versus-object mental model needed for later debugging. You also have one more AI boundary: not a ban, but a rule for keeping verification in the student's hands.

## Key Terms

- **attribute:** define this term in the context of the semester project before using it in lab work.
- **field:** define this term in the context of the semester project before using it in lab work.
- **method:** define this term in the context of the semester project before using it in lab work.
- **constructor:** define this term in the context of the semester project before using it in lab work.
- **reference:** define this term in the context of the semester project before using it in lab work.
- **stack:** define this term in the context of the semester project before using it in lab work.
- **heap:** define this term in the context of the semester project before using it in lab work.
- **instance:** define this term in the context of the semester project before using it in lab work.

## Bridge Question

Objects exist. How does a user move through them?

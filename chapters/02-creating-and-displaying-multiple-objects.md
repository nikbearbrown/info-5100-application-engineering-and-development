...
<!-- → [FIGURE: clean two-column diagram (Stack | Heap) showing patronA and patronB as labeled arrows pointing from stack variable boxes to separate heap object blocks containing name/age/libraryId fields — student should see that two names, two arrows, two blocks is the fundamental shape of independent objects] -->
...
<!-- → [FIGURE: stack/heap diagram showing the shared-reference situation — patronA and patronC both drawn as arrows converging on the same heap block, with a "99" written into the age field and a callout reading "both variables see this value" — contrast panel alongside showing patronA and patronB pointing to separate blocks to make the distinction vivid] -->
...
<!-- → [TABLE: four-row table mapping each artifact (class definition, constructor, instantiation, field access) to its role, the line of code that carries it, and what disappears from your understanding if you skip it — student should be able to use this as a self-check checklist] -->
...
<!-- → [TABLE: two-column comparison of display() vs toString() — columns: method, where it's called, who calls it, what it enables, when to use it — student should see that toString() is the wider-reaching choice and understand why without having it asserted] -->
...
<!-- → [INFOGRAPHIC: side-by-side panels — left: "What the printout shows" (terminal output with two patron lines, no structure visible); right: "What the trace shows" (stack/heap diagram with variable names, arrows, and field values annotated) — caption: the printout is the effect; the trace is the cause] -->
...
<!-- → [INFOGRAPHIC: horizontal flow diagram of the AI boundary rule — three labeled stages: (1) Student writes class body [human only], (2) Student asks AI: "Ask me five questions about this code" [AI as examiner], (3) Student answers without looking [verification] — with a gate symbol between stages 1 and 2 labeled "phase gate: write first"] -->

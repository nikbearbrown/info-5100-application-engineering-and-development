...
<!-- → [INFOGRAPHIC: annotated scene graph tree — Stage at root, Scene below it, BorderPane as center node, then TableView / SearchBar / DetailPanel as children; each node labeled with its type and what it knows (TableView: knows how to render rows, does not know checkout rules; SearchBar: knows user input, does not know catalog logic); helps student see the tree structure before writing any code] -->
...
<!-- → [TABLE: two-column decision guide — Display decision (belongs in handler) vs. Business decision (belongs in model); rows: "Which color to show for an available book", "Whether a patron is eligible to borrow", "Updating the table after a checkout", "Calculating the due date", "Disabling the checkout button when no row is selected", "Enforcing the three-book limit"; student uses this as a checklist when auditing their own event handlers] -->
...
<!-- → [INFOGRAPHIC: three-layer stack — Model (Book, Patron, checkout logic), Controller (event handlers, calls model methods, updates view), View (TableView, panes, controls); arrows showing the allowed directions of dependency (controller → model, controller → view, view does not call model directly); student should see which arrows are forbidden and why] -->

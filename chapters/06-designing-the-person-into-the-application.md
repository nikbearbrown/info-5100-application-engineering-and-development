...
<!-- → [INFOGRAPHIC: two-column diagram contrasting the conflated design (a single Patron class with fields: name, borrowHistory, passwordHash, username) vs. the separated design (Person class with name and borrowHistory linked by association arrow to UserAccount class with username and passwordHash, with LoginManager sitting alongside UserAccount and returning a boolean) — the visual emphasis should be on what each class knows and what it does not need to know] -->
...
<!-- → [IMAGE: annotated code walkthrough of the hash() method — annotations at each step: (1) MessageDigest.getInstance selects the algorithm; (2) getBytes converts the string to UTF-8 bytes — the charset matters; (3) digest() does the one-way computation; (4) the loop converts raw bytes to a readable hex string — this is the value you store and compare] -->
...
<!-- → [TABLE: threat model template — columns: threat scenario, what the attacker gains, what the plaintext design exposes, what the hashed design exposes, what would make the hashed design stronger; students complete this table for their own domain before writing authentication code] -->
...
<!-- → [TABLE: cross-domain comparison table — columns: domain, domain participant object, UserAccount fields, what the login manager returns, threat-model severity, what a file read exposes; rows: library (Patron), inventory (Employee), scheduling (Patient) — the purpose is to show that the design shape is identical while the stakes differ] -->
...
<!-- → [INFOGRAPHIC: sequence diagram of a complete login attempt — swim lanes: LoginPanel (submit handler), LoginManager (login method), UserAccount (checkPassword), HashMap (accounts directory); arrows show: (1) login(username, password) call from panel to manager; (2) accounts.get(username) call from manager to map; (3) checkPassword(supplied) call from manager to UserAccount; (4) hash(supplied) computed internally; (5) boolean returned up the chain — with a side annotation showing that Patron is never touched in any lane] -->

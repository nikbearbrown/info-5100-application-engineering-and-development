...
<!-- → [INFOGRAPHIC: vertical stack showing symptom → proximate cause → root cause with directional arrows; each level annotated with the library example (wrong output / wrong patron attached / currentPatron never reset); brief annotation on each level showing how far upstream the error lives and how detectable it is without a debugger] -->
...
<!-- → [TABLE: side-by-side comparison of print statement vs. breakpoint debugging — columns: property, print statement, breakpoint; rows: when you decide what to observe, how many values visible at once, can you follow a reference chain, can you navigate upstream, effort to add/remove — student should see that breakpoints extend, rather than replace, the print-statement intuition] -->
...
<!-- → [INFOGRAPHIC: three-phase loop — Hypothesize (name object, field, moment; state diverges here); Isolate (set breakpoint; pause execution; inspect state); Test (compare observation to prediction; confirm or revise); arrows looping back from Test to Hypothesize; annotation showing where AI enters (after hypothesis is stated) and where engineer judgment is non-delegable (the initial causal reading)] -->

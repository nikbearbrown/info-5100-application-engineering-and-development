...
<!-- → [TABLE: five-case taxonomy — columns: case name, what it tests, library example, inventory example, scheduling example; rows: normal case, empty case, no-match case, boundary case, invalid input case; student uses this as a checklist when designing their own test suite for any method] -->
...
<!-- → [CHART: horizontal timeline from Module 1 to Module 13; two lines — one for a student who writes tests module-by-module (tests accumulate, regression caught immediately when it occurs at Module 11); one for a student who tests only at Module 13 (no signal until final lab, multiple regressions hidden); student should see that test value compounds with time, not just at the moment of writing] -->
...
<!-- → [INFOGRAPHIC: test anatomy diagram — three-zone layout showing Setup (known initial state, labeled inputs), Execution (method call), Assertion (expected vs. actual, with arrows showing what happens on pass vs. fail); below it, a timeline showing the same test suite running across Module 1 through Module 13 as changes accumulate, with a regression highlighted at the point a new feature breaks old behavior] -->

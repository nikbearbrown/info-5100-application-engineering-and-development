...
**One-line:** Students learn a systematic debugging methodology - hypothesis, isolation, test - using the IDE debugger and their own reasoning, not error message reading alone.

## Module Overview

By the end of this module, you will add one concrete capability to the semester project: the chapter teaches debugging as causal diagnosis rather than output patching. The library example is the main path through the text. Inventory and healthcare scheduling follow the same design shape in the exercises.

## Prerequisites

Modules 1-3: classes, objects, references, method calls, and screen flow.

## Learning Objectives

- (Apply) Apply hypothesize, isolate, test to a hidden defect.
- (Analyze) Distinguish symptom, proximate cause, and root cause.
- (Apply) Use the NetBeans debugger to set breakpoints and inspect state.
- (Evaluate) Decide which debugging tasks are appropriate for AI.

## Opening Case

The program compiles and runs. It produces wrong output. There is no helpful error message. The student wants to change lines until the output looks right. The chapter asks for a hypothesis first.
...
Debugging is not guessing with better tools. It is a disciplined loop: name what you think is happening, isolate where evidence could confirm it, test that evidence, then revise.
...
AI may explain error categories or stack traces. It may not inspect code until the student states a hypothesis.
...
<!-- → [FIGURE: debugging workflow loop for Module 4, showing the core concept, the Java artifact that carries it, the AI assistance boundary, and the human verification step.] -->

## Worked Example

**Situation.** A checkout attaches a book to the wrong patron only after a specific sequence. The debugger stops before the assignment. The student inspects `currentPatron`, compares it to the expected patron, and finds the root cause upstream in a shared reference.
...
<!-- → [TABLE: Module 4 verification table with columns: concept, Java artifact, what AI may do, what the student must verify, evidence before submission; rows should include the library example plus inventory and scheduling variants.] -->
...
Find a hidden bug using the debugger. Submit the symptom, hypothesis, isolation step, evidence, fix, and what changed in your mental model.

The lab must include running evidence, a short explanation of the design choice, and the AI Use Disclosure described below.

## How To Use AI

**What AI does well here:** AI may explain error categories or stack traces. It may not inspect code until the student states a hypothesis.

**Concrete prompt that works:**

```text
I am working on Module 4: Finding Bugs: The Debugging Workflow in INFO 5100.
My domain is [library / inventory / scheduling].
My current specification is: [paste your written specification].
Help only within this boundary: AI may explain error categories or stack traces. It may not inspect code until the student states a hypothesis.
Do not produce work that violates the phase gate: No hypothesis, no AI. Show AI your hypothesis and ask whether it is plausible, not what the fix is.
```
...
**Phase gate:** No hypothesis, no AI. Show AI your hypothesis and ask whether it is plausible, not what the fix is.

**Module CLAUDE.md constraint:**

```text
You are assisting a graduate Java student in Module 4: Finding Bugs: The Debugging Workflow.
...
You can now the chapter teaches debugging as causal diagnosis rather than output patching. You also have one more AI boundary: not a ban, but a rule for keeping verification in the student's hands.

## Key Terms

- **symptom:** define this term in the context of the semester project before using it in lab work.
- **hypothesis:** define this term in the context of the semester project before using it in lab work.
- **isolation:** define this term in the context of the semester project before using it in lab work.
- **breakpoint:** define this term in the context of the semester project before using it in lab work.
- **step over:** define this term in the context of the semester project before using it in lab work.
- **step into:** define this term in the context of the semester project before using it in lab work.
- **root cause:** define this term in the context of the semester project before using it in lab work.

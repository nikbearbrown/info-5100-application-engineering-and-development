...
**One-line:** Students learn to connect the object model they have built to a visual interface - understanding that the GUI is a view of the model, not the model itself.

## Module Overview

By the end of this module, you will add one concrete capability to the semester project: the chapter separates model truth from visual representation. The library example is the main path through the text. Inventory and healthcare scheduling follow the same design shape in the exercises.

## Prerequisites

Module 9 searchable and sortable collections.

## Learning Objectives

- (Apply) Build a basic JavaFX application displaying domain objects.
- (Understand) Explain model, view, and controller separation.
- (Apply) Use JavaFX panes, controls, shapes, and TableView.
- (Evaluate) Assess GUI maintainability against MVC.

## Opening Case

The same catalog appears first as console output and then as a JavaFX table. The books did not change. The view did. The student must name what changed and what stayed in the model.
...
A GUI is a view onto object state, not a replacement for the object model. JavaFX gives a scene graph, controls, panes, and bindings. The engineer's job is to keep display decisions from swallowing business logic.
...
AI may generate JavaFX layout and display boilerplate. It may not generate event handlers or model logic.
...
<!-- → [FIGURE: MVC mapping diagram for Module 10, showing the core concept, the Java artifact that carries it, the AI assistance boundary, and the human verification step.] -->

## Worked Example

**Situation.** A `TableView<Book>` displays title, author, and availability from existing `Book` objects. A search bar filters the table by calling model or service methods, not by rewriting book data in the view.
...
<!-- → [TABLE: Module 10 verification table with columns: concept, Java artifact, what AI may do, what the student must verify, evidence before submission; rows should include the library example plus inventory and scheduling variants.] -->
...
Add a JavaFX catalog view to the semester project. Display real model data and trace every visible value back to an object field.

The lab must include running evidence, a short explanation of the design choice, and the AI Use Disclosure described below.

## How To Use AI

**What AI does well here:** AI may generate JavaFX layout and display boilerplate. It may not generate event handlers or model logic.

**Concrete prompt that works:**

```text
I am working on Module 10: Introduction to GUI Programming with JavaFX in INFO 5100.
My domain is [library / inventory / scheduling].
My current specification is: [paste your written specification].
Help only within this boundary: AI may generate JavaFX layout and display boilerplate. It may not generate event handlers or model logic.
Do not produce work that violates the phase gate: Every displayed value must be traced to a model field before submission.
```
...
**Phase gate:** Every displayed value must be traced to a model field before submission.

**Module CLAUDE.md constraint:**

```text
You are assisting a graduate Java student in Module 10: Introduction to GUI Programming with JavaFX.
...
You can now the chapter separates model truth from visual representation. You also have one more AI boundary: not a ban, but a rule for keeping verification in the student's hands.

## Key Terms

- **JavaFX:** define this term in the context of the semester project before using it in lab work.
- **scene graph:** define this term in the context of the semester project before using it in lab work.
- **pane:** define this term in the context of the semester project before using it in lab work.
- **control:** define this term in the context of the semester project before using it in lab work.
- **TableView:** define this term in the context of the semester project before using it in lab work.
- **model:** define this term in the context of the semester project before using it in lab work.
- **view:** define this term in the context of the semester project before using it in lab work.
